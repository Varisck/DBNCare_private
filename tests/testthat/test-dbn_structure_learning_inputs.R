set.seed(12321)
DBN <- DBNCare::random.structure.dbn(
  c("A","B","C"),
  prob_edge_intraslice = 0.5, 
  prob_edge_interslice= 0.5, 
  markov_order = 1)
DBN_fitted <- DBNCare::generate_dbn_nodes_distributions(DBN, TRUE, 2)
set.seed(123)
data <- DBNCare::dbn.sampling(DBN_fitted, 10, 5)

test_that("dbn.prep.data runs without error", {
  expect_no_error(DBNCare::dbn.prep.data(data))
})

test_that("dbn.learn.structure runs with default settings", {
  expect_no_error(DBNCare::dbn.learn.structure(data))
})
...
test_that("dbn.learn.structure accepts all score-based scores", {
  for (i in c("loglik", "aic", "bic", "ebic", "bde", "bds", "mbde", "k2", "fnml", "qnml", "nal", "pnal")) {
    expect_no_error(
      DBNCare::dbn.learn.structure(data, score = i)
    )
  }
  suppressWarnings(
    DBNCare::dbn.learn.structure(data, score = "bdla"))
})
...
test_that("dbn.learn.structure accepts all constraint-based tests via pc.stable", {
  for (i in c("mi", "mi-adf", "mc-mi", "smc-mi", "sp-mi", "mi-sh", "x2-adf", "mc-x2", "smc-x2", "sp-x2")) {
    expect_no_error(
      DBNCare::dbn.learn.structure(data, algorithm = "pc.stable", test = i)
    )
  }
})
...
test_that("dbn.learn.structure accepts score-based algorithms", {
  for (i in c("hc", "tabu")) {
    expect_no_error(
      DBNCare::dbn.learn.structure(data, algorithm = i)
    )
  }
})
