...
The Network at time 0 is visualized via the `plot` method from DBNCare with `network = "g_0"`:

```{r}
# Network at time 0
plot(diabetes_management_dbn, network = "g_0")
```

The same goes for the transition network, the default of `plot`:

```{r}
# Transition Network
plot(diabetes_management_dbn)
```
...
fitted_diabetes_management_dbn <- DBNCare::DBN_parameters(diabetes_management_dbn, CPT_list)
...
learned_diabetes_management_dbn <- DBNCare::StructureLearning.Network(data = sample_data_diabetes, algorithm = 'hc', score='bic')
...
plot(learned_diabetes_management_dbn, network = "g_0")
```

```{r}
# Visualization of the transition network for the learned DBN (via Hill Climbing method)
plot(learned_diabetes_management_dbn)
```
...
diabetes_management_dbn_PC <- DBNCare::StructureLearning.Network(data = sample_data_diabetes, algorithm = 'pc.stable', test='mi')
...
diabetes_management_dbn_MMHC <- DBNCare::StructureLearning.Network(data = sample_data_diabetes, algorithm = 'rsmax2', algorithm.res = "pc.stable", algorithm.max = "tabu", score='loglik',test='mi')
...
diabetes_management_dbn_HC_prior <- DBNCare::StructureLearning.Network(data = sample_data_diabetes, algorithm = 'hc', score='bic',blacklist=blacklist_set,whitelist=whitelist_set)
