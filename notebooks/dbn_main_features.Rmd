...
G_0 can be visualized by passing `network = "g_0"` to `plot`.

```{r}
plot(my_dbn, network = "g_0")
```
...
Using the function random.dbn the user can create a Dynamic Bayesian Network. The random generation of the network is made up of two steps:
...
dbn_at_random<-random.dbn(LETTERS[1:5],TRUE,0.6, 0.5)
...
For more details on how the random Dynamic Bayesian Network is generated, please read the documentation (*?random.dbn*, *?fit_random_dbn*, *?random.structure.dbn*, *?generate_dbn_nodes_distributions*).
