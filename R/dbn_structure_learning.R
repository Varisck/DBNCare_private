...
resolve_score <- function(score, data) {
  if (is.null(score) || identical(score, "auto")) {
    dbn_default_score(dataset_type(data))
  } else {
    score
  }
}
resolve_test <- function(test, data) {
  if (is.null(test) || identical(test, "auto")) {
    dbn_default_test(dataset_type(data))
  } else {
    test
  }
}
...
#' @param extend_g0 = TRUE bool weather to include in G_0 nodes with time < markov_order. If false G_0 will only have _0 nodes
#'
...
dbn.prep.data <- function(data, markov_order = 1, blacklist = NULL, whitelist = NULL,
  ...
  # names(df_0) = lapply(names(df_0), concat_name_pos``t, postfix = "_0")
  df_0 <- build_g0_df(data, markov_order = markov_order, extend_g0 = extend_g0)
  # removes the cols in exclude_cols
  df_0 <- select(df_0, -all_of("Sample_id"))
  ...
  # only keep t|t-i -> t|t-i nodes and t_[0-9]+ -> t_[0-9]+
  # in blacklist there are also A_0 -> A_t and stuff (not usefull for learning)
  blacklist_new <- blacklist_new[which(
    arr.ind = T,
    (grepl(
      "^.+(t|t-[0-9]+)$",
      blacklist_new[, "from"]
  ...
  # get all pairs in blacklist where from has _[0-9]+
  blacklist_0 <- blacklist[which(
    arr.ind = T,
    grepl("^.+(_[0-9]+)$", blacklist[, "from"])
  ), , drop = FALSE]
  # get all pairs in blacklist where from has _t(-i)
  blacklist_t <- blacklist[which(
    arr.ind = T,
    grepl("^.+(t|t-[0-9]+)$", blacklist[, "from"])
  ), , drop = FALSE]

  # do the same for the whitelist
  whitelist_0 <- whitelist[which(
    arr.ind = T,
    grepl("^.+(_[0-9]+)$", whitelist[, "from"])
  ), , drop = FALSE]
  whitelist_t <- whitelist[which(
    arr.ind = T,
    grepl("^.+(t|t-[0-9]+)$", whitelist[, "from"])
  ), , drop = FALSE]
...
dbn.build.struct <- function(PN, TN, markov_order = 1) {
  ...
  variables <- unique(unlist(lapply(names(PN$nodes), get_variable_name)))
  # creating DBN nodes list with the gt and g0
  for (variable in variables) {
    DBN$nodes[[variable]][["t"]] <- TN$nodes[[paste0(variable, "_t")]]

    # copy nodes in transition net for all markov orders
    for (mo in 1:markov_order) {
      DBN$nodes[[variable]][[paste0("t-", mo)]] <- TN$nodes[[paste0(variable, "_t-", mo)]]
    }

    DBN$nodes[[variable]][["type"]] <- "Dynamic"
  }
  # also include the g_0 network nodes
  for (variable in names(PN$nodes)) {
    name <- get_variable_name(variable)
    time <- get_variable_time(variable)
    DBN$nodes[[name]][[time]] <- PN$nodes[[variable]]
  }
...
#' @param extend_g0 = TRUE bool whether to include in G_0 nodes with time < markov_order. If false G_0 will only have _0 nodes
...
#' pc.stable.dbn(data)
#'
pc.stable.dbn <- function(
  data, markov_order = 1, test = "auto", max.sx = NULL,
  blacklist = NULL, whitelist = NULL, extend_g0 = TRUE, ...
) {
  test <- resolve_test(test, data)
  check_sl_params("constraint", test, NULL, max.sx)
  c(data0, dataTR, blacklist0, blacklistTR, whitelist0, whitelistTR) %<-% dbn.prep.data(
    data = data, markov_order = markov_order, blacklist = blacklist,
    whitelist = whitelist, extend_g0 = extend_g0
  )
  bn_sampled_0 <- pc.stable(
    data0,
    test = test, max.sx = min(max.sx, length(names(data0)) - 1),
    blacklist = blacklist0, whitelist = whitelist0, ...
  )
  bn_sampled_TR <- pc.stable(
    dataTR,
    test = test, max.sx = min(max.sx, length(names(dataTR)) - 1),
    blacklist = blacklistTR, whitelist = whitelistTR, ...
  )
...
fast.iamb.dbn <- function(
  ...
  bn_sampled_0 <- fast.iamb(
    data0,
    test = test, max.sx = min(max.sx, length(names(data0)) - 1),
    blacklist = blacklist0, whitelist = whitelist0, ...
  )
  bn_sampled_TR <- fast.iamb(
    dataTR,
    test = test, max.sx = min(max.sx, length(names(dataTR)) - 1),
    blacklist = blacklistTR, whitelist = whitelistTR, ...
  )
...
inter.iamb.dbn <- function(
  ...
  bn_sampled_0 <- inter.iamb(
    data0,
    test = test, max.sx = min(max.sx, length(names(data0)) - 1),
    blacklist = blacklist0, whitelist = whitelist0, ...
  )
  bn_sampled_TR <- inter.iamb(
    dataTR,
    test = test, max.sx = min(max.sx, length(names(dataTR)) - 1),
    blacklist = blacklistTR, whitelist = whitelistTR, ...
  )
...
iamb.fdr.dbn <- function(
  ...
  bn_sampled_0 <- iamb.fdr(
    data0,
    test = test, max.sx = min(max.sx, length(names(data0)) - 1),
    blacklist = blacklist0, whitelist = whitelist0, ...
  )
  bn_sampled_TR <- iamb.fdr(
    dataTR,
    test = test, max.sx = min(max.sx, length(names(dataTR)) - 1),
    blacklist = blacklistTR, whitelist = whitelistTR, ...
  )
...
si.hiton.pc.dbn <- function(
  ...
  bn_sampled_0 <- si.hiton.pc(
    data0,
    test = test, max.sx = min(max.sx, length(names(data0)) - 1),
    blacklist = blacklist0, whitelist = whitelist0, ...
  )
  bn_sampled_TR <- si.hiton.pc(
    dataTR,
    test = test, max.sx = min(max.sx, length(names(dataTR)) - 1),
    blacklist = blacklistTR, whitelist = whitelistTR, ...
  )
...
hpc.dbn <- function(
  ...
  bn_sampled_0 <- hpc(
    data0, test = test, max.sx = min(max.sx, length(names(data0)) - 1),
    blacklist = blacklist0, whitelist = whitelist0, ...
  )
  bn_sampled_TR <- hpc(
    dataTR, test = test, max.sx = min(max.sx, length(names(dataTR)) - 1),
    blacklist = blacklistTR, whitelist = whitelistTR, ...
  )
  dbn.build.struct(PN = bn_sampled_0, TN = bn_sampled_TR, markov_order = markov_order)
}
...
#'
hc.dbn <- function(
  data, markov_order = 1, score = "auto", max.sx = NULL,
  blacklist = NULL, whitelist = NULL, extend_g0 = TRUE, ...
) {
  ...
  bn_sampled_0 <- hc(
    data0, score = score, maxp = min(max.sx, length(names(data0)) - 1),
    blacklist = blacklist0, whitelist = whitelist0, ...
  )
  bn_sampled_TR <- hc(
    dataTR, score = score, maxp = min(max.sx, length(names(dataTR)) - 1),
    blacklist = blacklistTR, whitelist = whitelistTR, ...
  )
  dbn.build.struct(PN = bn_sampled_0, TN = bn_sampled_TR, markov_order = markov_order)
}
...
#'
h2pc.dbn <- function(
  data, markov_order = 1, score = "auto", test = "auto", max.sx = NULL,
  blacklist = NULL, whitelist = NULL, extend_g0 = TRUE, ...
) {
  score <- resolve_score(score, data)
  test <- resolve_test(test, data)
  ...
  bn_sampled_0 <- h2pc(
    data0,
    maximize.args = list(score = score, maxp = min(max.sx, length(names(data0)) - 1)),
    restrict.args = list(test = test, max.sx = min(max.sx, length(names(data0)) - 1), ...),
    blacklist = blacklist0, whitelist = whitelist0
  )
  bn_sampled_TR <- h2pc(
    dataTR,
    maximize.args = list(score = score, maxp = min(max.sx, length(names(dataTR)) - 1)),
    restrict.args = list(test = test, max.sx = min(max.sx, length(names(dataTR)) - 1), ...),
    blacklist = blacklistTR, whitelist = whitelistTR
  )
  dbn.build.struct(PN = bn_sampled_0, TN = bn_sampled_TR, markov_order = markov_order)
}
...
#'
rsmax2.dbn <- function(
  data, markov_order = 1, restrict = "pc.stable", maximize = "hc",
  score = "auto", test = "auto", max.sx = NULL,
  blacklist = NULL, whitelist = NULL, extend_g0 = TRUE, ...
) {
  score <- resolve_score(score, data)
  test <- resolve_test(test, data)
  ...
  bn_sampled_0 <- rsmax2(
    data0, restrict = restrict, maximize = maximize,
    maximize.args = list(score = score, maxp = min(max.sx, length(names(data0)) - 1)),
    restrict.args = list(test = test, max.sx = min(max.sx, length(names(data0)) - 1), ...),
    blacklist = blacklist0, whitelist = whitelist0
  )
  bn_sampled_TR <- rsmax2(
    dataTR, restrict = restrict, maximize = maximize,
    maximize.args = list(score = score, maxp = min(max.sx, length(names(dataTR)) - 1)),
    restrict.args = list(test = test, max.sx = min(max.sx, length(names(dataTR)) - 1), ...),
    blacklist = blacklistTR, whitelist = whitelistTR
  )
...
dbn.learn.structure <- function(
  data, markov_order = 1, algorithm = "hc",
  algorithm.res = "pc.stable", algorithm.max = "hc",
  blacklist = NULL, whitelist = NULL, test = NULL, score = NULL,
  max.sx = NULL, extend_g0 = TRUE, ...
) {
  if (algorithm %in% c("hc", "tabu")) {
    get(paste(algorithm, ".dbn", sep = ""))(
      data = data, markov_order = markov_order, score = score,
      blacklist = blacklist, whitelist = whitelist,
      max.sx = max.sx, extend_g0 = extend_g0, ...
    )
  } else if (algorithm %in% c("pc.stable", "gs", "iamb", "fast.iamb", "inter.iamb", "iamb.fdr")) {
    get(paste(algorithm, ".dbn", sep = ""))(
      data = data, markov_order = markov_order, test = test,
      blacklist = blacklist, whitelist = whitelist,
      max.sx = max.sx, extend_g0 = extend_g0, ...
    )
    ...
    get(paste(algorithm, ".dbn", sep = ""))(
      data = data, markov_order = markov_order, score = score, test = test,
      blacklist = blacklist, whitelist = whitelist,
      max.sx = max.sx, extend_g0 = extend_g0, ...
    )
  } else if (algorithm == "rsmax2" & algorithm.res %in% c("pc.stable", "gs", "iamb", "fast.iamb", "inter.iamb", "iamb.fdr") & algorithm.max %in% c("hc", "tabu")) {
    get(paste(algorithm, ".dbn", sep = ""))(
      data = data, markov_order = markov_order,
      restrict = algorithm.res, maximize = algorithm.max,
